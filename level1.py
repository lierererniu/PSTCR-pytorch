...
def PSTCR(gtlist, is_real, modelpath, log_name, out_dir, outpath):
    validationreport = TestReport(log_dir=r'E:\cloudremove\PSTCR\Evaluation', log_name=log_name)
    avg_rmse = 0
    avg_psnr = 0
    avg_ssim = 0
    avg_cc = 0
    ...
    for name in gtlist:
        ...
        if is_real == True:
            ...
            bh = '.bmp'
        ori_mask_1 = (
                    1 - cv2.imread(r'D:\cloudromove\real\sentinel-2\20180816_mask_\\' + name[:-4] + bh, cv2.IMREAD_GRAYSCALE) / 255)
        ori_mask_2 = (
                    1 - cv2.imread(r'D:\cloudromove\real\sentinel-2\20180905_mask_\\' + name[:-4] + bh, cv2.IMREAD_GRAYSCALE) / 255)
        ori_mask_3 = (
                    1 - cv2.imread(r'D:\cloudromove\real\sentinel-2\20180915_mask_\\' + name[:-4] + bh, cv2.IMREAD_GRAYSCALE) / 255)
        ori_mask_4 = (
                    1 - cv2.imread(r'D:\cloudromove\real\sentinel-2\20180925_mask_\\' + name[:-4] + bh, cv2.IMREAD_GRAYSCALE) / 255)
        ori_mask_5 = (
                    1 - cv2.imread(r'D:\cloudromove\real\sentinel-2\20181020_mask_\\' + name[:-4] + bh, cv2.IMREAD_GRAYSCALE) / 255)
        ...
        while np.where(ori_mask_1 == 0)[0].size != 0:
            iter_1 = iter_1 + 1
            # 迭代一次
            ...
            for x in range(0, w - patch + 1, stride):
                for y in range(0, w - patch + 1, stride):
                    # 无云像素
                    intact_numbers = np.where(ori_mask_1[x: x + patch, y: y + patch] == 1)[0].size
                    # 无需去云
                    # if intact_numbers == patch * patch:
                    if intact_numbers == patch * patch or intact_numbers < patch * patch * ratio:
                        continue
                    else:  # 需要填充
                        All_Temp[:, :, 0] = Original_2[x: x + patch, y: y + patch]
                        All_Temp[:, :, 1] = Original_3[x: x + patch, y: y + patch]
                        All_Temp[:, :, 2] = Original_4[x: x + patch, y: y + patch]
                        All_Temp[:, :, 3] = Original_5[x: x + patch, y: y + patch]

                        ALL_Mask[:, :, 0] = ori_mask_2_1[x: x + patch, y: y + patch]
                        ALL_Mask[:, :, 1] = ori_mask_3_1[x: x + patch, y: y + patch]
                        ALL_Mask[:, :, 2] = ori_mask_4_1[x: x + patch, y: y + patch]
                        ALL_Mask[:, :, 3] = ori_mask_5_1[x: x + patch, y: y + patch]
                        Cc_temp_1 = Cc_Value(Cloud_1[x: x + patch, y: y + patch], All_Temp[:, :, 0]
                                             * ori_mask_1[x: x + patch, y: y + patch])
                        Cc_temp_2 = Cc_Value(Cloud_1[x: x + patch, y: y + patch], All_Temp[:, :, 1]
                                             * ori_mask_1[x: x + patch, y: y + patch])
                        Cc_temp_3 = Cc_Value(Cloud_1[x: x + patch, y: y + patch], All_Temp[:, :, 2]
                                             * ori_mask_1[x: x + patch, y: y + patch])
                        Cc_temp_4 = Cc_Value(Cloud_1[x: x + patch, y: y + patch], All_Temp[:, :, 3]
                                             * ori_mask_1[x: x + patch, y: y + patch])
                        ...
                        parameter = Temporal_Linear_Fit2(Cloud_1[x: x + patch, y: y + patch],
                                                         All_Temp[:, :, pos_max[3]],
                                                         ori_mask_1[x: x + patch, y: y + patch],
                                                         ALL_Mask[:, :, pos_max[3]])

                        # 输入网络的图1
                        Cloud_patch = Cloud_1[x: x + patch, y: y + patch]
                        _ = Cloud_1[x: x + patch, y: y + patch]
                        # #####
                        mask_patch = 1 - ori_mask_1[x: x + patch, y: y + patch]
                        ...
                        Res_patch = output * mask_patch + _ * ori_mask_1[x: x + patch, y: y + patch]
            ...
            print("Iteration: {}, rest of missing Regions = {:.3f}%, takes {} seconds. ".format(iter_1, rest_ratio,
                                                                                                round(t_time, 4)))
        # print('1 avg test time:', (time.time() - start_time))

        ssim = SSIM(Temp_1, Cloud_1)
        psnr = peak_signal_noise_ratio(Temp_1, Cloud_1)
        cc, rmse = Evaluation_index(Cloud_1, Temp_1)
        ...
        save_bmp(Cloud_1, out_dir + outpath + '/' + 'result' + '/result_' + name)
        save_bmp(Temp_1, out_dir + outpath + '/' + 'gt' + '/gt_' + name)
        save_bmp(Original_1, out_dir + outpath + '/' + 'cloud' + '/cloud_' + name)
...
if __name__ == '__main__':
    # gtlist = os.listdir(r'C:\Users\53110\Desktop\农田\20211121')
    ...
    masklist = os.listdir(r'D:\cloudromove\real\sentinel-2\20180816_mask_')
    masklist.sort(key=lambda x: int(x[:-4]))
    modelpath = r'E:\cloudremove\result\PSTCR\models\best.pth'
    outdir = r'E:\cloudremove\PSTCR\\'
    outpath = 's2-11'
    start_time = time.time()
    PSTCR(gtlist, True, modelpath, log_name='s2-11', out_dir=outdir, outpath=outpath)
